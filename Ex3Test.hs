...
revProg = 
    ...
    , MkRule "revProg"
            ...
            (EV "etc")
    , MkRule "revProg"
            [PC "Cons" [PV "head", PV "etc"]]
            -- (head:etc) -> (etc:head)
            (EC "Cons" [EA "append" [EV "etc", EV "head"]])
    ]

{- Also write a test case for this program and the expected output here:

Used plusProgramAST and my appendProgramAST from Ex3 as a similar template for rev
but changed slightly to satisfy rev.
...
{- 3.3.1 A "Panic" function.
   ...
   Write the changes you have made here:
            ...
            Altered case in eval:  env (EA f args)  
    
            I have left in the comments my original case for this before changing it to check for "panic".
            Changed so that the pattern looks at (a:etc) instead of "variable".

            Second test case works for me but for first test case, the closing parenthesis is in the incorrect place.
            Tried to write a helper function which does similar to ppValue but for a list of values. However this
            didn't work so I reverted to using ppValue.
