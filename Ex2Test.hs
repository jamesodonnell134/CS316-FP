...
applyPredicate v = map (\x -> if filter(v) then [True] else [False])
...
sumLengths :: [[a]] -> Int
sumLengths (x:xs) = foldr (\x, sum -> if (x == []) then sum + 0, len + 0 else sum + 1, len + 1 sumlengths xs) 0
...
flatten :: Tree a -> [a]
flatten Leaf x          = [x]
flatten (Node lt x rt)  = foldTree Leaf (\l x r -> [x])
...
pair comp = \a1 a2 -> case is
     if comp a1 a2  == LT then comp ((a1,b1) (a2,b2))
     if comp a1 a2 == GT then comp ((a1,b1) > (a2,b2))
     if comp a1 a2 == EQ then comp ((a1,b1) && (a2,b2)))
...
flipTopBottom pic = \x y -> if(y >= 0) then (pic(x,-y)) else if(y <= 0) (pic(x,-y))

flipLeftRight :: Picture a -> Picture a
flipLeftRight pic = \x y -> if(x >= 0) then (pic(-x,y)) else if(x <= 0) (pic(-x,y))

    -- Check horizontal vals, double negate to get positive value
...
chatBot = Output "Hello! What is your name?\n" (Input (\name -> Output "Hello" (Output name (End ()))))
-- Was not sure how to get to stay on the same line
-- chatBot = Output "Hello! What is your name?\n" (Input (\name ->Output name (End ()))) 
