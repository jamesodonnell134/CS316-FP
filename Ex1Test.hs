...
chunk :: Int -> [a] -> [a] -> [[a]]
chunk num acc []        = []
chunk num acc (x:xs)    = (acc:x) chunk
...
removeEmpties :: [[a]] -> [[a]]
removeEmpties [[]] = [[]]
removeEmpties (x:xs) = (if x ==[]) then (removeEmpties xs) else xs
...
delete (AtEnd [])                       = (AtEnd [])
--delete (AtEnd after)                    = (AtEnd after)
delete (Within before point [])         = (AtEnd before)
delete (AtEnd (x:xs))                   = (AtEnd (x:xs))
delete (Within (x:xs) point after)      = (Within (x:xs) point [])
...
now :: Undoable -> Cursor
now (Undoable past present future) = (Within before point after) -- Fill this in

update :: Undoable -> Cursor -> Undoable
update (Within before point []) = (Undoable present past [])
...
duplicate :: Process
duplicate = Input ((Output True End) (Output False End)) (Output False End)
...
flipOutputs Input (Output True End) (Output False End) = (Input (Output False End) (Output True End))
...
flipInputs Input (Output False End) (Output True End) = Input (Output True End)
flipInputs Input (Output True End) (Output True End) = Input (Output False End)
