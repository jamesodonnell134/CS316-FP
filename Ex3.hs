...
pProgram = 
          do
            ...
            return (rules)
            `orElse`
            failParse "Error parsing program!"
...
hasMain prog = case findRule "main" [] prog of
                Ok (a) -> Ok ()
                Error a -> Error "No main function in program!"
...
scopeCheck [] =  Ok ()
scopeCheck prog = do
                  traverse ruleCheck prog
                  return ()

ruleCheck:: Rule -> ErrorOr ()
ruleCheck (MkRule string pattern expression) =
  do
