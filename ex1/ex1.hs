...
{- 1.1.0 Concatenation of lists. The infix operator ++ concatenates
   ...
   -- concatLists is of type (list of lists) outputting a list
concatLists :: [[x]] -> [x]
concatLists []      = []
...
goodProp c xs = joinWith c (splitOn c xs) == xs
...
{- 1.1.6 Finding things in trees.
  ...
  -- eqFindInTree 2 (Node (Node Leaf 4 Leaf) 2 Leaf) == TRUE
  -- eqFindInTree 1 (Node (Node Leaf 4 Leaf) 2 Leaf) == TRUE
...
{- 1.2.0 Querying a cursor. Write two functions that 
(a) Test whether or not a cursor is at the end of the line; -}

atEnd :: Cursor -> Bool
atEnd (AtEnd _) = True
atEnd (Within before point after) = if after == [] then True else False

{-(b) Test whether or not a cursor is at the start of the line.-}  

atStart :: Cursor -> Bool
atStart (AtEnd _) = False
atStart (Within before point after) = if before == [] then True else False
...
{- 1.2.1 Movement. Here is a function that moves the cursor one step to
   ...
   list in reverse: it makes moving the cursor into a quick operationpoint
...
moveLeft (AtEnd [])                       = AtEnd []
moveLeft (AtEnd (x:before))               = Within before x []
moveLeft (Within [] point after)          = Within [] point after
moveLeft (Within (x:before) point after)  = Within before x (point:after)
...
insert x (Within (before) (point) (after))  = (Within (x:before) (point) (after))
...
backspace (Within [] point after)          = (Within [] point after)
backspace (AtEnd (x:xs))                   = (AtEnd xs)
backspace (Within (x:xs) point after)      = (Within xs point after)
...
execute ((MoveRight):xs)  cursor            = execute xs (moveRight cursor)
execute ((MoveLeft):xs) cursor              = execute xs (moveLeft cursor)
execute ((Insert c):xs) cursor              = execute xs (insert c cursor)
execute ((Backspace):xs) cursor             = execute xs (backspace cursor)
execute ((SearchRightFor c):xs) cursor      = execute xs (searchRightFor c cursor)
...
-- [True values down left side. False outputs down right side].
-- [Negate the notGate function to get the same output].
